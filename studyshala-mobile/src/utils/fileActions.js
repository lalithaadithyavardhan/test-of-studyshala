...
export const openFile = async (file, material, navigation) => {
  ...
  const localFile = await fileRepository.getById(file._id);
  if (localFile?.localPath && localFile?.downloaded) {
    // Open from local storage — no internet needed
    if (navigation) {
      const parentNav = navigation.getParent() || navigation;
      parentNav.navigate('FileViewer', { file: { ...file, localPath: localFile.localPath }, material });
      return;
    }
  }
