...
export default function FileViewerScreen({ route, navigation }) {
  ...
  // Session tracking
  useEffect(() => {
    if (file?._id && material?._id) {
      sessionTracker.startSession(file._id, material._id);
      preloadService.recordOpen(file._id, material._id).catch(() => {});
    }
    return () => {
      if (file?._id) {
        sessionTracker.endSession(file._id, 0).catch(() => {});
      }
    };
