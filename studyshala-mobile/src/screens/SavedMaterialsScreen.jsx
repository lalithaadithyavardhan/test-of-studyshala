...
export default function SavedMaterialsScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // Check which are saved offline locally
      const offlineMaterials = await materialRepository.getAllSaved();
      setOfflineIds(new Set(offlineMaterials.map(m => m.materialId)));
    } catch {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <MaterialCard
              ...
              dark
              offlineStatus={offlineIds.has(item._id?.toString()) ? 'saved' : null}
            />
          )}
          ListEmptyComponent={
