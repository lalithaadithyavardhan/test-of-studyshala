...
import { downloadFile, openFile } from '../utils/fileActions';
...
export default function MaterialAccessScreen({ route, navigation }) {
  ...
  const checkOfflineStatus = async () => {
    const local = await materialRepository.getById(initialMaterial._id);
    if (local?.savedOffline) setOfflineSaved(true);
  };

  const checkForUpdates = async () => {
    try {
      const local = await materialRepository.getLocalVersion(initialMaterial._id);
      if (!local) return;
      const updates = await syncService.checkAllForUpdates();
      const hasUpdate = updates.some(u => u.materialId === initialMaterial._id);
  ...
  const handleSaveOffline = async () => {
    ...
    try {
      const forecast = await downloadManager.getForecast(files);
      if (!forecast.willFit) {
        Alert.alert('Not enough storage', `This material needs ${Math.round(forecast.requiredSize / 1024 / 1024)} MB but you only have ${Math.round(forecast.freeStorage / 1024 / 1024)} MB free.`);
        return;
      }

      await materialRepository.upsert({
        materialId: initialMaterial._id,
        subject: initialMaterial.subjectName,
        facultyName: initialMaterial.facultyName,
        department: initialMaterial.department,
        semester: initialMaterial.semester,
        accessCode: initialMaterial.accessCode,
        version: 1,
      });
      ...
      await downloadManager.saveOffline(
        initialMaterial._id,
        files,
        async (driveFileId) => {
          const f = files.find(fl => fl.driveFileId === driveFileId);
          return f?.downloadUrl || '';
        },
        (fileId, percent) => console.log(`${fileId}: ${percent}%`)
      );

      setOfflineSaved(true);
      Alert.alert('Saved Offline ✓', `${initialMaterial.subjectName} is now available without internet.`);
    } catch (e) {
      Alert.alert('Error', e.message || 'Failed to save offline.');
    } finally {
...
function WorkspaceHeader({ material, totalFiles, totalFolders, saving, onBack, onSave, onSaveOffline, offlineSaving, offlineSaved, updateAvailable }) {
  return (
    <>
      <View style={s.heroCard}>
        <View style={s.heroTopRow}>
          ...
          <TouchableOpacity style={s.saveBtn} onPress={onSaveOffline} disabled={offlineSaving}>
            {offlineSaving
              ? <ActivityIndicator size="small" color="#4ade80" />
              : <Ionicons name={offlineSaved ? "cloud-done" : "cloud-download-outline"} size={18} color={offlineSaved ? "#4ade80" : C.accent} />
            }
          </TouchableOpacity>
        </View>
