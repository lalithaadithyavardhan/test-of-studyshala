  ...
  try {
    ...
    res.json({
      ...
      material: {
        ...
        // FIX: include driveViewLink in file list for preview
        files: folder.files.map(f => ({
          _id: f._id,
          name: f.name,
          mimeType: f.mimeType,
          size: f.size,
          driveViewLink: f.driveViewLink || null,
...
const getMaterialFiles = async (req, res) => {
  try {
    ...
    // FIX: Check access - must be in savedMaterials OR accessHistory
    const hasAccess = 
    ...
    res.json({
      ...
      // FIX: include driveViewLink so frontend can offer inline preview
      files: folder.files.map(f => ({
        _id: f._id,
        name: f.name,
        mimeType: f.mimeType,
...
const downloadFile = async (req, res) => {
  try {
    ...
    // FIX: Access check - student must have saved or accessed via code
    const hasAccess = 
      req.user.savedMaterials.some(m => m.materialId.toString() === id) ||
      req.user.accessHistory.some(h => h.materialId.toString() === id);

    if (!hasAccess) {
      return res.status(403).json({ message: 'Access denied. Please enter the access code first.' });
    }
