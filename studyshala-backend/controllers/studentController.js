...
const getMaterialVersion = async (req, res) => {
  try {
    ...
    const folder = await Folder.findOne({ _id: id, active: true })
      .select('version files subFolders subjectName updatedAt');
    if (!folder) return res.status(404).json({ message: 'Material not found' });
    ...
    const serverVersion = folder.version || 1;
    const hasUpdate = localVersion ? serverVersion > parseInt(localVersion) : false;
